...
import { X, Github, ExternalLink, Cpu, Network, Brain, Database, Shield, GitBranch, Layers, Activity, Zap } from 'lucide-react';
...
const techStack = [
  ...
  { name: 'Three.js', desc: '3D Visualization', color: '#000000' },
...
export function AboutPage({ isActive, onClose }: AboutPageProps) {
  ...
  return (
    <motion.div
      ...
      style={{ background: 'rgba(9,9,11,0.95)', backdropFilter: 'blur(30px)' }}
      onClick={onClose}
              ...
              <div className="mt-6">
                ...
                <div className="flex gap-4">
                  {fraudPatterns.map((p) => (
                    <button
                      key={p.name}
                      onClick={(e) => { e.stopPropagation(); setActivePattern(p.name.toLowerCase().split(' ')[0]); }}
