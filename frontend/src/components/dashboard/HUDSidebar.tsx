...
export function HUDSidebar({ alerts, selectedId, onSelect, isCollapsed, onToggle, side }: HUDSidebarProps) {
      ...
      {!isCollapsed ? (
        ...
        <div className="flex flex-col items-center pt-4 gap-4">
          <div className="flex flex-col items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full" style={{ background: '#f43f5e', boxShadow: '0 0 8px #f43f5e' }} />
            <span className="text-xs font-bold" style={{ color: '#f43f5e' }}>{criticalAlerts.length}</span>
          </div>
          <div className="flex flex-col items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full" style={{ background: '#f59e0b' }} />
            <span className="text-xs font-bold" style={{ color: '#f59e0b' }}>{suspiciousAlerts.length}</span>
          </div>
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
